...
Note: the legacy ``Code/utils/confusion_matrix.py`` used ``matrix[p, t]`` (row =
predicted, col = true). This module uses the standard convention
``matrix[true, pred]`` (rows = true labels, columns = predictions), which makes
per-class recall/FP/FN arithmetic direct and the plotted heatmap intuitive.
"""
...
def evaluate_predictions(preds: np.ndarray, labels: np.ndarray, num_classes: int) -> dict[str, float]:
    ...
    fp = matrix.sum(axis=0) - tp  # predicted but actually other classes
    fn = matrix.sum(axis=1) - tp  # actual but predicted as other classes
