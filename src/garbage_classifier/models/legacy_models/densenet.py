"""DenseNet (2017) - 层间稠密连接。

学习要点：
  - 每一层都接收前面所有层的特征（通道拼接），梯度因此能直接流到浅层。
  - 相比 ResNet 的加法连接，拼接保留了更多信息；参数效率极高。
  - 代价：显存占用高（需要保存中间特征用于反向传播）。
"""
...
# 其中，Any表示任何类型，List表示列表类型，
# Tuple表示元组类型。这些类型可以用于函数参数、返回值等地方，以提供静态类型检查的能力。
from typing import Any, List, Tuple

import torch
import torch.nn as nn
import torch.nn.functional as F
# 通过使用checkpoint模块，可以将模型的某些层的计算结果临时保存到磁盘上，
# 而不是一直保留在内存中，从而减少了内存的使用量。这对于训练一些需要大量内存的深度学习模型非常有用。
...
class _DenseLayer(nn.Module):
    ...
    # 这个函数是一个使用了PyTorch JIT的脚本函数，用于调用一个检查点瓶颈（checkpoint bottleneck）操作。
    # 它接受一个输入列表，将这些输入传递给内部定义的闭包函数，并通过PyTorch的checkpointing机制来运行闭包函数，
    # 最后返回一个张量作为输出。@torch.jit.unused注释表示这个函数不会被JIT编译器使用，但仍然可以被调用和执行。
    @torch.jit.unused
