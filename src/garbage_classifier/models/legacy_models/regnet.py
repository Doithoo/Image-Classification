"""RegNet (2020) - 用"设计空间搜索"代替人工设计网络宽度/深度。

学习要点：
  - 把网络设计变成参数化公式（宽度 w0、深度 d、组宽 g 等），在公式空间里搜索，
    得到规整且高效的 RegNetX/Y 系列。
  - 参数化的好处：同一条公式可以生成不同规模的网络（200MF 到 16GF）。
"""
...
class RegNet(nn.Module):
    ...
    def _build_stage_info(cfg: dict):
        ...
        return [dict(out_c=stage_widths[i],
                     depth=stage_depths[i],
                     group_width=stage_groups[i],
                     se_ratio=cfg["se_ratio"]) for i in range(num_stages)]


def regnet(model_name="RegNetX_200MF", num_classes=1000):
