...
class Trainer:
    ...
    def __init__(
        ...
        t = cfg.train
        self.class_weights_tensor: torch.Tensor | None
        if class_weights:
        ...
        if t.scheduler in ("cosine", "step"):
            if t.scheduler == "cosine":
                base: Any = torch.optim.lr_scheduler.CosineAnnealingLR(
    ...
    def fit(self, train_loader: DataLoader, valid_loader: DataLoader, resume_from: str | None = None) -> dict[str, Any]:
        ...
        if resume_from is not None:
            ...
            csv_logger.reconcile_epoch(self.epoch)
        ...
        for epoch in range(self.epoch, self.cfg.train.epochs):
            ...
            self.epoch = epoch + 1
            score = metrics[self.cfg.train.best_metric]
            ...
            if self.cfg.train.early_stop_patience > 0 and self.patience_left <= 0:
    ...
    def _resume(self, path: str) -> None:
        ...
        validate_resume_identity(
            payload,
            build_resume_identity(self.cfg, self.class_names, self.manifest_identity),
        )
